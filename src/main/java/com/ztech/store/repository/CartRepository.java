...
public interface CartRepository extends R2dbcRepository<Cart, Long>, CartRepositoryInternal {
    @Query("SELECT * FROM cart entity WHERE entity.customer_id = :id")
    Flux<Cart> findByCustomer(Long id);

    @Query("SELECT * FROM cart entity WHERE entity.customer_id IS NULL")
    Flux<Cart> findAllWhereCustomerIsNull();
