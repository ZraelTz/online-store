...
public interface CartItemRepository extends R2dbcRepository<CartItem, Long>, CartItemRepositoryInternal {
    ...
    @Query("SELECT * FROM cart_item entity WHERE entity.customer_id = :id")
    Flux<CartItem> findByCustomer(Long id);

    @Query("SELECT * FROM cart_item entity WHERE entity.customer_id IS NULL")
    Flux<CartItem> findAllWhereCustomerIsNull();
