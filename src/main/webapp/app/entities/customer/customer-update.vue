    ...
    <div class="col-8">
      <form name="editForm" role="form" novalidate v-on:submit.prevent="save()">
        ...
        <div>
          ...
          <div class="form-group">
            <label class="form-control-label" v-text="$t('storeApp.customer.countryState')" for="customer-countryState"
              >Country State</label
            >
            <input
              type="text"
              class="form-control"
              name="countryState"
              id="customer-countryState"
              data-cy="countryState"
              :class="{ valid: !$v.customer.countryState.$invalid, invalid: $v.customer.countryState.$invalid }"
              v-model="$v.customer.countryState.$model"
              required
            />
            <div v-if="$v.customer.countryState.$anyDirty && $v.customer.countryState.$invalid">
              <small class="form-text text-danger" v-if="!$v.customer.countryState.required" v-text="$t('entity.validation.required')">
                This field is required.
              </small>
            </div>
          </div>
